...
public class PC {
    ...
    public PC(Clock clock, DriveSet drives, Calendar startTime) throws IOException {
        this(clock, drives, DEFAULT_RAM_SIZE, startTime, new PCConfig());
    }

    public PC(Clock clock, DriveSet drives) throws IOException {
        this(clock, drives, DEFAULT_RAM_SIZE, getStartTime(), new PCConfig());
    }
     ...
     */
    public PC(Clock clock, DriveSet drives, int ramSize, Calendar startTime, PCConfig config) throws IOException {
    ...
    public PC(Clock clock, DriveSet drives, int ramSize) throws IOException {
       this(clock, drives, ramSize, getStartTime(), new PCConfig());
    }
    ...
    public void sendMouse(Integer x, Integer y, Integer z, Integer dx, Integer dy, Integer dz, Integer buttons)
    {
        keyboard.putMouseEvent(x, y, z, dx, dy, dz, buttons);
    }
...
//                    throw new IllegalStateException("new address reached "+Integer.toHexString(processor.getInstructionPointer()));
	...
	public void add(HardwareComponent c) {
		parts.add(c);
	}
}
